...
VALUES
  ...
  -- Number of unique nodes in bank system
SELECT COUNT(DISTINCT node_id) AS UniqueNodes_count
FROM customer_nodes;

-- Number of nodes per region
SELECT r.region_id, r.region_name, COUNT(DISTINCT n.node_id) as Number_of_nodes
FROM regions r
LEFT JOIN customer_nodes as n ON r.region_id = n.region_id
GROUP BY r.region_id, r.region_name;

-- Number of customers per region
SELECT r.region_id, r.region_name, COUNT(n.customer_id) as customers_count, COUNT(DISTINCT n.customer_id) as unique_customers
FROM regions AS r
LEFT JOIN customer_nodes AS n ON r.region_id = n.region_id
GROUP BY r.region_id, r.region_name;

-- Average duration of each allocation 
SELECT * 
FROM customer_nodes
WHERE customer_id =1 		-- Realsing they have a node ending in year 9999, which is forever
AND end_date <> '9999-12-31' 		-- Remove it as an outlier
LIMIT 100;
...
SELECT ROUND(AVG(days_in_node),0) as avg_days_reallocation -- ROUND IT UP TO 0 decimals
FROM days_in_node
LIMIT 100;

-- Median, 80th and 95th percentile for this same reallocation days metric for each region
-- This is for counting rows to find median (50th rows), 80th and 95th
WITH days_in_node AS(
	SELECT region_name, customer_id, node_id, SUM(DATEDIFF(end_date, start_date)) as days_in_node
...
ORDERED AS (
SELECT region_name, days_in_node, ROW_NUMBER() OVER (PARTITION BY region_name ORDER BY days_in_node) as rn 
FROM days_in_node),
MAX_RN AS (
...
-- MEDIAN
SELECT o.*, m.max_rn, ROUND(m.max_rn/2,0) AS median_row,
	CASE 
		WHEN rn = ROUND(m.max_rn/2,0) THEN 'Median'
        WHEN rn = ROUND(m.max_rn*0.8,0) THEN '80th percentile'
        WHEN ROUND(m.max_rn*0.95,0) THEN '95th percentile'
	END AS metric
FROM ORDERED AS o
INNER JOIN MAX_RN AS m on m.region_name = o.region_name
WHERE rn IN(
 ROUND(m.max_rn/2,0),
...
SELECT ct.txn_type, COUNT(ct.customer_id) AS unique_trans_count, SUM(ct.txn_amount) AS total_amount
...
WITH deposit_transactions AS (
	SELECT ct.customer_id, COUNT(*) AS deposit_count, AVG(ct.txn_amount) as total_deposit_amount
	FROM customer_transactions as ct
	WHERE ct.txn_type = 'deposit'
	GROUP BY ct.customer_id
)
SELECT ROUND(AVG(deposit_count),0) AS average_deposit_count, ROUND(AVG(total_deposit_amount),3) AS average_deposit_amount
FROM deposit_transactions;

-- For each month - how many Data Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
WITH conditions AS (
	SELECT ct.customer_id, DATE_FORMAT(txn_date, '%Y-%m') AS month, 
		SUM(CASE WHEN txn_type = 'deposit' THEN 1 ELSE 0 END) as deposit_count,
		SUM(CASE WHEN txn_type <> 'deposit' THEN 1 ELSE 0 END) as withdrawal_or_purchase
	FROM customer_transactions AS ct
	GROUP BY ct.customer_id, DATE_FORMAT(txn_date, '%Y-%m')
...
running_balance AS (
  SELECT 
    ...
    txn_type,
    balance,
    SUM(balance) OVER (PARTITION BY customer_id, end_of_month ORDER BY txn_date) AS running_sum,
    ROW_NUMBER() OVER (PARTITION BY customer_id, end_of_month ORDER BY txn_date DESC) AS rn
  FROM conditions
)
SELECT 
  customer_id,
...
closing_balance AS (
  SELECT
    customer_id,
    txn_month,
    SUM(net_monthly_change) OVER (PARTITION BY customer_id ORDER BY txn_month ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS closing_balance
...
growth_flagged AS (
  SELECT *,
         CASE 
           WHEN prev_balance IS NOT NULL AND prev_balance > 0 AND (closing_balance - prev_balance)/prev_balance > 0.05
           THEN 1 ELSE 0 
         END AS grew_by_5_percent
  FROM balance_with_lag
)
SELECT 
  ROUND(100 * (SUM(grew_by_5_percent) / COUNT(*)), 2) AS percent_grew_by_5
FROM growth_flagged;
